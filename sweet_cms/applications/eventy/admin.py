...
def register_routes(blueprint):
    ...
    def events_programme_view(programme_id):
        ...
        # )
        programmefile=ProgrammeFile.query.first()
        ...
        return render_template(themed_template('eventy/programme.html', 'admin'), programme=programme, programme_form=programme_form,
                               programme_data_form=programme_data_form,programmefile=programmefile)
    
    
    # Live Stream all programme
    @blueprint.route('/live/stream/for_user/<int:programme_id>', methods=['POST'])
    @admin_required
    def live_stream_programm_now(programme_id):
        new_status = request.form['banner']
        
        

        program_data = ProgrammeFile.query.filter_by(programme_id=programme_id).all()
        for status in program_data:
            status.file_status = new_status
            db.session.add(status)
            db.session.commit()
        flash("Streaming All Videos SuccessFully.", 'success')
        return redirect(url_for('admin.events_programme_view', programme_id=programme_id))
#new function
    @blueprint.route('/live/stream/for_users/<int:programme_id>', methods=['POST'])
    @admin_required
    def live_stop_programm_now(programme_id):
        
        new = request.form['que']
        ...
        for status in program_data:
            ...
            db.session.add(status)
            db.session.commit()
        flash(" All streaming stoped Videos SuccessFully.", 'success')
        return redirect(url_for('admin.events_programme_view', programme_id=programme_id)) 
    ...
    # update live streaming for user ..
    @blueprint.route('/eventy/programme/<int:programme_id>/live_video/<int:programme_file_id>', methods=['POST', 'GET'])
    @admin_required
    def live_stream_for_users(programme_id, programme_file_id):
        programme = Programme.query.get_or_404(programme_id)
        programme_file = ProgrammeFile.query.filter_by(programme=programme).filter_by(id=programme_file_id).filter_by(file_type="video").first()
        # Debugger stop here 
        if not programme_file:
            abort(404)
        stream = request.form['stream']
        # for file_item in programme_file:
        #     print("zkjsdfhksdkfj", programme_file_id)
        #     if file_item.id == "banner":
        #         file_item.file_type = stream
        #         db.session.add(programme_file)
        #         db.session.commit()
        
        programme_file.file_status = stream
        db.session.add(programme_file)
        db.session.commit()
        flash("Programme file Updated successfully", 'success')
        return redirect(url_for('admin.events_programme_view', programme_id=programme_id))
    ...
    @blueprint.route('/eventy/delete/<int:programme_id>')
    @admin_required        
    def events_programme_delete(programme_id):
        
        programme = Programme.query.get_or_404(programme_id)
        db.session.delete(programme)
        db.session.commit()
        flash("list deleted sucessfully")
